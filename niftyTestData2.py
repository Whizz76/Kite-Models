...
index_data=pd.read_csv("Jun22ToJun23NiftyIndex.csv")
...
time_step = timedelta(minutes=5)  # Assuming 1-minute intervals

# Define the range for stoploss (percentage)
min_percentage = 20
max_percentage = 100
percentage_step = 5  # Assuming 1% intervals
...
for time in time_values:
    ...
    for stoploss_val in stoploss_values:
        ...
        for it in range(len(index_data)):
            ...
            if(len(expiry_date)==0):
                ...
                folder_path="../Nifty/Nifty/"+expiry_day
                ...
                for it in range(0,5):
                    ...
                    call_path="Nifty"+input_date_string+str(strike_price)+"CE.csv"
                    put_path="Nifty"+input_date_string+str(strike_price)+"PE.csv"
                    if(is_file_present(folder_path,call_path) and is_file_present(folder_path,put_path)):
                        break

                for it in range(1,6):
                    strike_price_500=price+(it*100)
                    call_path="Nifty"+input_date_string+str(strike_price_500)+"CE.csv"
                    put_path="Nifty"+input_date_string+str(strike_price_500)+"PE.csv"
                    if(is_file_present(folder_path,call_path) and is_file_present(folder_path,put_path)):
                        break
                ...
                for itr in range(0,2):
                    ...
                    if(is_file_present(folder_path,call_path) and is_file_present(folder_path,put_path)):
                        ...
                        call_data=pd.read_csv("../Nifty/Nifty/"+expiry_day+"/Nifty"+input_date_string+str(strike_price)+"CE.csv")
                        put_data=pd.read_csv("../Nifty/Nifty/"+expiry_day+"/Nifty"+input_date_string+str(strike_price)+"PE.csv")
                        ...
                        for i in range(len(call_data_1)):
                            ...
                            cur_call_data=call_data_1.loc[i,"close"]*(1+stoploss)
                            call_stop=min(cur_call_data,call_stop)
                        ...
                        for i in range(len(put_data_1)):
                            ...
                            cur_put_data=put_data_1.loc[i,"close"]*(1+stoploss)
                            put_stop=min(cur_put_data,put_stop)
    ...
    folder_path="outputFiles/Nifty2"
    csv_file_name=f'Nifty_01Jun22_To_30Aug22_{time}.csv'
    csv_file_name_100=f'Nifty_01Jun22_To_30Aug22_{time}_100.csv'
