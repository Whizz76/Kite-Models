...
# Define the range for profit (percentage)
min_percentage = 5
max_percentage = 50
...
# Storing the profit values

profit_values = list(range(min_percentage, max_percentage + 1, percentage_step))
...
for time in time_values:
    ...
    for profit_val in profit_values:

        profit=profit_val*0.01
            ...
            if(len(expiry_date)==0):
                ...
                for itr in range(0,2):
                    ...
                    if(is_file_present(folder_path,call_path) and is_file_present(folder_path,put_path)):
                        ...
                        call_stop=call_start*(1+profit)
                        put_stop=put_start*(1+profit)
                        ...
                        call_profit=None
                        put_profit=None
                        call_profit_percent=None
                        put_profit_percent=None
                        ...
                        for i in range(len(call_data_1)):
                            ...
                            if(call_data_1.loc[i,"close"]>=call_stop):
                                ...
                                call_profit_percent=((call_stop-call_start)/call_start)*100
                                call_profit=call_stop-call_start
                        ...
                        for i in range(len(put_data_1)):
                            ...
                            if(put_data_1.loc[i,"close"]>=put_stop):
                                ...
                                put_profit_percent=((put_stop-put_start)/put_start)*100
                                put_profit=put_stop-put_start
                        ...
                        data_output_val=[date,expiry_date,time,profit*100,strike_price,actual_strike_price,call_start,call_stop,call_profit,call_profit_percent,call_stop_time,
                                            put_start,put_stop,put_profit,put_profit_percent,put_stop_time,net,net_gain,net_loss]
                        
                        if(data_output_val in output_data):
                            continue

                        print(net,time,profit)
    ...
    columns=["date","expiry_date","time","profit","strike_price","actual_strike_price","call_start","call_stop","call_profit","call_profit_percent","call_stop_time","put_start",
                                            "put_stop","put_profit","put_profit_percent","put_stop_time","net","net_gain","net_loss"]
