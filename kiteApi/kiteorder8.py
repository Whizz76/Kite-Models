            ...
            if(place_limit_order(cur_price,LTP) and buy_id!=None and not_triggered):
                if(num_mod<23):
                    ...
                    if(buy_id): 
                        LTP=cur_price
                        num_mod+=1
                        logging.info("modifications done {} current_sym {}".format(num_mod,tradingSym_ATM))
                else:
                    temp_id=buy_id
...
def on_ticks(ws, ticks):
  ...
  for tick in ticks:
    ...
    if(trade_data['sl_reached_PE'] and trade_data['sl_reached_CE']):
        ...
        if(tick['instrument_token']==instrument_syms[trade_data['token']]): trade_data['SP']=tick["last_price"]
            ...
            # if(trade_data['tradingSym_PE_ATM'] and tick['instrument_token']==instrument_syms[trade_data['tradingSym_PE_ATM']]): 
            #     trade_data['cur_PE_price']=tick['last_price']
            #     print("PE cur price ",trade_data['cur_PE_price']," ",datetime.now())

            # elif(trade_data['tradingSym_CE_ATM'] and tick['instrument_token']==instrument_syms[trade_data['tradingSym_CE_ATM']]): 
            #     trade_data['cur_CE_price']=tick['last_price']
            #     print("CE cur price ",trade_data['cur_CE_price']," ",datetime.now())
        ...
        print("SP: ",trade_data['SP']," TIME ",datetime.now())
    ...
    if(tick['instrument_token']!=token_PE and tick['instrument_token']!=token_CE): 
        print("token absent: ",tick['instrument_token'])
        continue

    if(tick['instrument_token']==token_PE): 
        print("for token_PE ",token_PE," ",tick['exchange_timestamp'])
        trade_data['cur_PE_price']=tick['last_price'] #buy_id,LTP,limit,trigger,sl_reached
        print("PE cur price ",trade_data['cur_PE_price']," ",datetime.now())

    if(tick['instrument_token']==token_CE): 
        print("for token_CE ",token_CE," ",tick['instrument_token'])
        trade_data['cur_CE_price']=tick['last_price']
        print("CE cur price ",trade_data['cur_CE_price']," ",datetime.now())
                       

  if(trade_data['sl_reached_PE'] and trade_data['sl_reached_CE'] and trade_data['SP']):
    ...
    if(trade_data['PE_OTM_buy_order']==None): 
        print("placing the buy order")
        trade_data['PE_OTM_buy_order']=place_order(trade_data['tradingSym_PE_OTM'],"buy",kite_exchange,kite.ORDER_TYPE_MARKET,kite.PRODUCT_MIS,quantity)
    if(trade_data['CE_OTM_buy_order']==None): 
        print("placing the buy order")
        trade_data['CE_OTM_buy_order']=place_order(trade_data['tradingSym_CE_OTM'],"buy",kite_exchange,kite.ORDER_TYPE_MARKET,kite.PRODUCT_MIS,quantity)
  ...
  print("----------------------------------SL BUY----------------------------------------")
  if(trade_data['cur_PE_price']): trade_data['PE_buy_id'],trade_data['PE_LTP'],trade_data['limit_PE'],trade_data['trigger_PE'],trade_data['sl_reached_PE']=buy_sl_order(trade_data['PE_buy_id'],trade_data['PE_LTP'],trade_data['cur_PE_price'],trade_data['tradingSym_PE_ATM'],trade_data['limit_PE'],
                                                                       trade_data['sl_reached_PE'],trade_data['trigger_PE'],quantity)
          
  print("PE_LTP {} limit_PE {} trigger_PE {} sl_reached_PE {} time {}".format(trade_data['PE_LTP'],trade_data['limit_PE'],trade_data['trigger_PE'],trade_data['sl_reached_PE'],datetime.now()))
  
  
  if(trade_data['cur_CE_price']): trade_data['CE_buy_id'],trade_data['CE_LTP'],trade_data['limit_CE'],trade_data['trigger_CE'],trade_data['sl_reached_CE']=buy_sl_order(trade_data['CE_buy_id'],trade_data['CE_LTP'],trade_data['cur_CE_price'],trade_data['tradingSym_CE_ATM'],trade_data['limit_CE']
                                                                       ,trade_data['sl_reached_CE'],trade_data['trigger_CE'],quantity)
  print("CE_LTP {} limit_CE {} trigger_CE {} sl_reached_CE {} time {}".format(trade_data['CE_LTP'],trade_data['limit_CE'],trade_data['trigger_CE'],trade_data['sl_reached_CE'],datetime.now()))
  print("--------------------------------SL BUY---------------------------------")     
  time.sleep(1)
