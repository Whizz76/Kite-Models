...
def place_order_time(time_hour,time_minute):
    ...
    if(placeOrder):
        ...
        tradingSym_PE_750=symbol+str(PE_price)+"PE"
        tradingSym_CE_750=symbol+str(CE_price)+"CE"

        PE_750=place_order(tradingSym_PE_750,"buy",kite.EXCHANGE_NFO,kite.ORDER_TYPE_MARKET,kite.PRODUCT_MIS)
        CE_750=place_order(tradingSym_CE_750,"buy",kite.EXCHANGE_NFO,kite.ORDER_TYPE_MARKET,kite.PRODUCT_MIS)

        PE_LTP=None
        CE_LTP=None

        if(PE_750 and CE_750): 
        ...
        while True:
            ...
            if(PE_stoploss_orderid!=None and CE_stoploss_orderid!=None):
                ...
                if(PE_750): PE_sell_order=place_order(tradingSym_PE_750,"sell",kite.EXCHANGE_NFO,kite.ORDER_TYPE_MARKET,kite.PRODUCT_MIS)
                if(CE_750): CE_sell_order=place_order(tradingSym_CE_750,"sell",kite.EXCHANGE_NFO,kite.ORDER_TYPE_MARKET,kite.PRODUCT_MIS)
